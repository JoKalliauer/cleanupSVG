...
for file in *.svg
 ...
 time taskset -c 0 resvg "$file" "${file%.svg}_rendersvg.png" --languages 'de-AT, de-DE, de'
 ...
 echo system-librsvg
 LANG=de_AT.UTF-8 time taskset -c 0 rsvg-convert "$file" >"${file%.svg}_librsvg.png"
 ...
 LANG=de_AT.UTF-8 time taskset -c 0 inkscape "$file" --export-type="png"
 ##inkscape "$file" --export-png="${file%.svg}_ink092.png"
 mv "${file%.svg}.png" "${file%.svg}_Inkscape.png"

 #echo batik
 #time taskset -c 0 java -jar ~/prgm/batik-1.14/batik-rasterizer-1.14.jar "$file" -d "${file%.svg}_batik.png" --lang 'de'
