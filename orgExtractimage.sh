...
do
 if [ -f "$fileSource" ]; then    
  ...
  if [ "$outputType" = "png" ];then
   ...
   inkscape --verb=EditSelectAll --verb=SelectionUnGroup --verb=SelectionUnGroup --verb=SelectionUnGroup --verb=SelectionUnGroup --verb=org.ekips.filter.extractimage.noprefs --verb=FileSave --verb=FileClose ./${fileSource} --verb=FileQuit
